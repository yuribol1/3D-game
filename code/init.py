...
class Drawing:
    def __init__(self, sc, sc_map, player, clock):
        ...
        self.textures = {1: pygame.image.load('img/m-001.png').convert(),
                         2: pygame.image.load('img/m-002.png').convert(),
                         3: pygame.image.load('img/m-022.png').convert(),
                         4: pygame.image.load('img/m-040.png').convert(),
        ...
        # weapon parameters
        self.weapon_base_sprite = pygame.image.load('sprites/weapon/ashot/shot/0.png').convert_alpha()
        self.weapon_shot_fire_animation = deque(
            [pygame.image.load(f'sprites/weapon/ashot/shot_fire/{i}.png').convert_alpha()
             for i in range(4)])
        self.weapon_shot_animation = deque([pygame.image.load(f'sprites/weapon/ashot/shot/{i}.png').convert_alpha()
                                            for i in range(4)])
        self.weapon_reload_animation = deque([pygame.image.load(f'sprites/weapon/ashot/reload/{i}.png').convert_alpha()
                                              for i in range(2)])
        self.sfx = deque([pygame.image.load(f'sprites/weapon/ashot/wall_shot/{i}.png').convert_alpha()
                          for i in range(3)])
        self.weapon_rect = self.weapon_base_sprite.get_rect()
        self.weapon_pos = (HALF_WIDTH - self.weapon_rect.width // 2, HEIGHT - self.weapon_rect.height)
        self.shot_fire_length = len(self.weapon_shot_fire_animation)
        self.shot_fire_length_count = 0
        self.shot_fire_animation_speed = 10
        self.shot_fire_animation_count = 0
        self.shot_fire_animation_trigger = True
        ...
        self.reload_animation_speed = 50
        self.reload_animation_count = 0
        self.reload_animation_trigger = True
        self.reload_sound = pygame.mixer.Sound('sound/reload.mp3')
        self.sfx_length_count = 0
        self.sfx_animation_speed = 10
        self.sfx_animation_count = 0
    ...
    def mini_map(self, player):
        self.sc_map.fill(WHITE)
        map_x, map_y = player.x // MAP_SCALE, player.y // MAP_SCALE
        pygame.draw.line(self.sc_map, YELLOW, (map_x, map_y), (map_x + 12 * math.cos(player.angle),
                                                 map_y + 12 * math.sin(player.angle)), 2)
        pygame.draw.circle(self.sc_map, RED, (int(map_x), int(map_y)), 5)
        for x, y in mini_map:
            pygame.draw.rect(self.sc_map, BLACK, (x, y, MAP_TILE, MAP_TILE))
        self.sc.blit(self.sc_map, MAP_POS)

    def player_weapon(self, shots):
        if self.player.shot_fire:
            shot_fire_sprite = self.weapon_shot_fire_animation[0]
            self.sc.blit(shot_fire_sprite, self.weapon_pos)
            self.shot_fire_animation_count += 1
            if self.shot_fire_animation_count == self.shot_fire_animation_speed:
                self.weapon_shot_fire_animation.rotate(-1)
                self.shot_fire_animation_count = 0
                self.shot_fire_length_count += 1
                self.shot_fire_animation_trigger = False
            if self.shot_fire_length_count == self.shot_fire_length:
                self.player.shot_fire = False
                self.shot_fire_length_count = 0
                self.shot_fire_animation_trigger = True
        if self.player.shot:
        ...
        elif self.player.reload:
            if not self.reload_length_count:
                self.reload_sound.play()

            reload_sprite = self.weapon_reload_animation[0]
            self.sc.blit(reload_sprite, self.weapon_pos)
            ...
            if self.reload_animation_count == self.reload_animation_speed:
                ...
                self.reload_animation_trigger = False
            if self.reload_length_count == self.reload_length:
                self.player.reload = False
                self.reload_length_count = 0
                self.reload_animation_trigger = True
    ...
    def win(self):
        ...
        pygame.draw.rect(self.sc, WHITE, rect, border_radius=50)
    ...
    def menu(self):
        ...
        while self.menu_trigger:
            ...
            color = randrange(40)
            label = label_font.render('3DGAME', 1, (color, color, color))
